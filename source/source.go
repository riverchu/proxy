...
// var ProxySourceList = [...]proxy.Source{localFile, fate0ProxyList, proxy66, mimvp, kxdaili, dieniao}

func init() { proxy.RegisterSource(localFile, fate0ProxyList, proxy66, mimvp, kxdaili, dieniao) }
...
type source struct {
	...
	getProxy   func(*source) *proxy.Proxy
	getProxies func(*source) proxy.ProxyArray
	parseProxy func(data io.Reader) proxy.ProxyArray
}
...
func (s *source) GetProxy() *proxy.Proxy                     { return s.getProxy(s) }
func (s *source) GetProxies() proxy.ProxyArray               { return s.getProxies(s) }
func (s *source) ParseProxy(data io.Reader) proxy.ProxyArray { return s.parseProxy(data) }

func (s *source) JudgeQuality() proxy.QualityLevel { return s.GetProxies().JudgeQuality() }

var (
	localFile = &source{
		name: "local_file",
		getProxy: func(s *source) (proxy *proxy.Proxy) {
		...
		getProxies: func(s *source) (proxies proxy.ProxyArray) {
			f, err := os.Open("./conf/proxy.json")
			if err != nil {
				return nil
			}
		...
		parseProxy: func(data io.Reader) (proxies proxy.ProxyArray) { return nil },
	}
	...
	fate0ProxyList = &source{
		...
		parseProxy: func(data io.Reader) (proxies proxy.ProxyArray) {
